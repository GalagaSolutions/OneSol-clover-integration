...
async function registerPaymentProvider(locationId, accessToken, liveMode) {
  ...
  const endpointAttempts = [
    {
      ...
      url: "https://services.leadconnectorhq.com/payments/custom-provider/config",
      payload: {
        locationId,
        liveMode: !!liveMode,
        apiKey: keys.apiKey,
  ...
  for (const attempt of endpointAttempts) {
    console.log(`🔧 Trying provider endpoint: ${attempt.name}`);
    try {
      const response = await axios.post(attempt.url, attempt.payload, { headers: baseHeaders });
      console.log(`✅ Provider configuration updated via ${attempt.name}`);
      ...
      lastError = error;
      console.warn(`⚠️ Provider endpoint failed: ${attempt.name}`);
      console.warn("   Status:", error.response?.status);
      console.warn("   Error:", JSON.stringify(error.response?.data));

      // Continue through known fallback endpoints for 404/405 mismatches.
      if (error.response?.status === 404 || error.response?.status === 405) {
        continue;
      }

      // For non-404/405 failures, stop early and return precise diagnostics.
      break;
    }
  }
  ...
  throw withStage(lastError, "provider_config_api", {
    locationId,
    liveMode: !!liveMode,
    status: lastError?.response?.status,
    responseData: lastError?.response?.data,
    hint: "All known LeadConnector provider config endpoints failed. Verify API contract for this account.",
  });
}
